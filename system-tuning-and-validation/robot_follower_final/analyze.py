...
def compute_metrics(data):
    ...
    t   = t - t[0]              # make plots relative to capture start
    d   = data[:, 1]             # dist_cm
    e   = data[:, 2]             # error_cm
    vb  = data[:, 3]             # v_base
    pL  = data[:, 4]             # pwm_L
    ...
    # ── rise time and settling time — ignore first 3 seconds of startup noise ──
    STARTUP_IGNORE_S = 3.0   # ignore first 3s (sensor warmup / filteredDist init)
    valid = t > STARTUP_IGNORE_S

    t_v = t[valid]
    e_v = e[valid]
    d_v = d[valid]

    # Rise time: first time error crosses into ±10% band AFTER a large error
    target_band = DESIRED_CM * 0.10
    large_error  = np.where(np.abs(e_v) > target_band * 3)[0]   # must have been outside first
    in_band      = np.where(np.abs(e_v) < target_band)[0]

    if len(large_error) > 0 and len(in_band) > 0:
        # only count crossings that happen AFTER a large error
        first_large = large_error[0]
        after_large = in_band[in_band > first_large]
        rise_time = t_v[after_large[0]] - t_v[first_large] if len(after_large) > 0 else None
    else:
        rise_time = None

    # Settling time: last time error is outside 5% band
    band_5pct = DESIRED_CM * 0.05
    outside   = np.where(np.abs(e_v) > band_5pct)[0]
    settle_time = t_v[outside[-1]] - t_v[0] if len(outside) > 0 else 0.0
...
# ── 1. Distance vs time — fix y-axis and rise/settle calculation ──
    ...
    # Fix: show full range including 80cm no-obstacle value
    ax1.set_ylim(0, max(np.max(m["dist"]) * 1.1, 90))  # ← ADD THIS
